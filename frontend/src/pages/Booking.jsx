    ...
    return (
      <section data-testid="booking-success" className="relative gradient-hero min-h-[80vh] flex items-center">
        <div className="max-w-3xl mx-auto px-6 py-20 text-center">
          ...
          <h1 className="font-display text-4xl md:text-5xl font-black tracking-tighter text-navy-900">
            You're on the list.
          </h1>
          <p className="mt-5 text-slate-600 text-lg max-w-xl mx-auto leading-relaxed">
            Thanks, <span className="font-semibold text-navy-900">{form.full_name}</span> — one of our payments specialists
            will reach out within 24 hours to confirm your slot{date ? ` on ${date.toDateString()}${time ? ` at ${time}` : ""}` : ""}.
          </p>
          <div className="mt-8 flex items-center justify-center gap-3">
            <Button
              data-testid="return-home-btn"
              onClick={() => navigate("/")}
              className="rounded-full bg-navy-900 hover:bg-navy-800 text-white h-11 px-6"
            >
              Back to home
            </Button>
            <Button
              variant="outline"
              className="rounded-full h-11 px-6 border-slate-300"
        ...
        <div className="grid md:grid-cols-12 gap-10 md:gap-16 items-start">
          ...
          <div className="md:col-span-7 animate-fade-up" style={{ animationDelay: "120ms" }}>
            <form
              ...
              onSubmit={onSubmit}
              className="relative rounded-3xl bg-white border border-slate-200 hard-shadow p-6 md:p-10"
            >
                ...
                <div className="md:col-span-2">
                  ...
                  <Select value={form.reason} onValueChange={(v) => update("reason", v)}>
                    <SelectTrigger
                      ...
                      className="mt-2 h-12 rounded-xl bg-slate-50 border-slate-200 focus:ring-indigo-500 focus:ring-2"
                    >
                      <SelectValue placeholder="What do you need help with?" />
                    </SelectTrigger>
                    <SelectContent className="rounded-xl">
                      {REASONS.map((r) => (
                        <SelectItem key={r.value} value={r.value} data-testid={`reason-${r.value}`}>
                          {r.label}
                        </SelectItem>
                      ))}
                    </SelectContent>
